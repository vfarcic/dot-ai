...
**Field naming is load-bearing here.** The GrafanaGhost disclosure ([Noma Security](https://noma.security/blog/grafana-ghost/), Sasi Levi, disclosed 2026-04-07) reports that embedding the keyword **`INTENT`** inside an injected payload was part of what made the target model treat planted instructions as authoritative. This project's field is named `intent`. The primary disclosure is linked because it is the source that names the keyword; [CyberScoop's coverage](https://cyberscoop.com/grafanaghost-grafana-prompt-injection-vulnerability-data-exfiltration/) says only "a specific keyword" and does not support the claim on its own.
...
The exposure matters **only when `rbac.enforcement.enabled` is `true`** — the chart default is `false`, and with RBAC off every authenticated caller could invoke `remediate` anyway, so the ungated reads grant nothing extra. Enabling RBAC is exactly when the engine starts making the promise the read layer then breaks. Ownership checks are not currently possible at all: `GenericSession` records no creator identity. Filed as a follow-up and ranked above several of the nine below; it is a concrete authorization bypass rather than a residual-framing gap.
