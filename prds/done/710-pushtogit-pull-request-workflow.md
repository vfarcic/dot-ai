...
6. **No mode-selection switch ships.** Resolved question 1 removed the only candidate (`gitops.pullRequestMode`), so PR mode is selected by the client parameter and enforced by RBAC, with no configuration governing it. **This is narrower than it originally read.** As written it claimed the work adds "no config surface at all", and that stopped being true on 2026-08-05 when the scope expanded to close the credential-exposure findings: `gitops.allowedRepoHosts` ships as a first-class chart value (see the update under resolved question 3). The rule below is what that value was built to satisfy. The project is Kubernetes-only, so `charts/values.yaml` is the configuration interface and a new parameter is a first-class value rendered into the container env by `charts/templates/deployment.yaml`, exactly as `rbac.enforcement.enabled` renders `DOT_AI_RBAC_ENABLED` (`deployment.yaml:174-177`). The env var is an internal implementation detail with no user-facing contract, and documenting a new parameter under `extraEnv` is not an acceptable interim step. Note that `DOT_AI_GIT_CREATE_DRAFT_PRS` (`internal-tools.ts:365-370`) moves into the extracted helper unchanged — it is a pre-existing test-only switch, not a new parameter, and must not acquire a chart value.
...
- ~~Repository allowlisting, and with it the credential-exposure problem behind it (resolved question 3) — related, separately motivated, needs its own issue. PR mode does not mitigate it.~~ **Reversed on 2026-08-05: this now ships in this PRD.** During review, two further instances of the same credential-exposure class were found alongside it, and the decision was taken to close all of them here rather than defer. See the note under resolved question 3.
...
**Progress as of 2026-08-06: all six milestones complete.** Shipped as PR [#716](https://github.com/vfarcic/dot-ai/pull/716). Integration groups `recommend` (6/6), `rbac` (34/34) and `prompts` (57/57) green locally, and all eight CI groups green on the PR; unit suite 920 passing.

The scope grew substantially after M3. Review of the feature surfaced six pre-existing security issues, all live in shipped v1.25.0 and none regressions of this work, and the decision was taken to fix them here rather than defer: a confirmed **arbitrary command execution** path (a write into `<clone>/.git/config` executed by `pushRepo`'s own next git call, while the push reported success); a **symlink write-escape** past a lexical-only containment guard; the **server's credential delivered to a caller-named host** across four call paths, closed by `gitops.allowedRepoHosts`; a **working credential no scrub pattern masked** (a PAT used as a URL username); a **catastrophic-backtracking ReDoS** in `scrubCredentials` (110 KB → 64 s of frozen main thread); and an **SSRF read primitive** against internal services, narrowed by refusing non-public IP literals. That is why this branch touches credential handling and path validation far beyond what the milestones below describe.
...
- [x] **M5 — Docs.** Tool docs (PR-mode example), authorization docs (verb rule for the two modes, the four enforcement limits, the client-reconnect note, and the upgrade paragraph), chart comments; regenerate OpenAPI; breaking-change changelog fragment in `changelog.d/`. All delivered, plus documentation for the credential-exposure fixes the scope grew to include. **Two** fragments ship, not one: the RBAC apply-verb change, and the git-credential host allowlist.
- [x] **M6 — ~~File the follow-up issue~~ SUPERSEDED: the allowlist ships here instead.** The milestone existed because the PRD deferred repository allowlisting. On 2026-08-05 that deferral was reversed and the fix was implemented in this PRD, together with two further instances of the same credential-exposure class (see resolved question 3). There is no follow-up issue left to file for it. Two *unrelated* pre-existing issues found during the same review are still worth filing separately: remediate's contradictory `'Automatic validation has been completed'` string, and the `charts/Chart.yaml`/`Chart.lock` pin of qdrant 1.18.0 against a committed 1.15.5 tarball, which makes every integration run dirty the tree.
...
3. **Repository allowlist — out of scope, but the stated reason for deferring was wrong and the follow-up issue needs reframing.** The original framing was that `repoUrl` is client-controlled against a shared token, so any authenticated user can push anywhere that token can reach, and that the reporter's repo-scoped `GithubAccessToken` via External Secrets handles it. It bounds the blast radius; it does not close the hole. `getAuthenticatedUrl` (`git-utils.ts:64-68`) embeds the server's token as HTTP basic auth into whatever URL the client supplied, and `pushToGit` validates `targetPath` but never the host — so any user with `execute` on `recommend` can name `https://attacker.example/x.git` and have the server deliver `DOT_AI_GIT_TOKEN`, or a GitHub App installation token, to a host they control. A repo-scoped token narrows what the attacker gains but is still a live credential for that repository, and the token also lands in the clone's `.git/config`.
   ...
   **Update 2026-08-05 — this was reversed and the fix ships in this PRD.** The reasoning below stands as the analysis; only the "out of scope" conclusion changed. A `gitops.allowedRepoHosts` chart value (default `["github.com"]`, exact hostname match, empty list = deny-all) now gates `pushToGit` **before any credential is minted or attached**, covering all four consumers of the client URL: `cloneRepo`, `pushRepo`'s origin rewrite, `createPullRequest`, and `lookupPullRequest`. Two further instances of the same class were found during the same review and also fixed: a PAT used as a URL *username* (`//ghp_…@host`) was masked by no scrub pattern, despite being a working GitHub credential that reached logs, session files, and responses; and the user-prompts repo override (PRD #581/#621) attached the server's credential to a client-supplied URL whose host was never validated — that one now degrades to an unauthenticated clone rather than being refused, so public repos are unaffected and private repos on non-allowlisted hosts use the `X-Dot-AI-Git-Token` header PRD #621 built.

   **Update 2026-08-06 — remediate's `git_clone` IS gated after all, and the gate also requires `https:`.** Two changes to the paragraph above, both from the security review of the fixes:

   - `git_clone` was left ungated on the reasoning that its URL is "AI-derived from cluster state rather than client-supplied". That premise is only half right: the URL is a model decision whose context includes the caller's free-text `issue` (`remediate.ts:337`) plus cluster objects a tenant may control, and `git_clone` is exposed during **investigation**, which has no approval step (only `git_create_pr` is executor-only). So a caller who can run `remediate` could steer the server's credential to a host they name inside the issue text, with no client parameter for it. It now degrades exactly like the prompts override (`withholdServerCredential: !isRepoHostAllowed(repoUrl)`), so public GitOps repos on any host keep cloning. The cost, and the one thing an operator must know: remediate has **no** `X-Dot-AI-Git-Token` escape hatch, so a **private** GitOps repository on a non-allowlisted host now needs that host added to `gitops.allowedRepoHosts` — the same one-line operator change as breaking change #2, on a second path. Still ungated at push time: `git_create_pr` pushes to the `origin` the clone recorded, which is behind the executor-only approval step, and gating it would break the feature for exactly the non-GitHub GitOps remotes it exists to serve.
   - The gate now requires the `https:` scheme, not just an allowed host. Host-only let `http://github.com/…`, `git://github.com/…` and `ssh://github.com/…` through to `getAuthenticatedUrl`, which embeds the token — basic-auth material on a cleartext request for `http://`, and an SSH username for `ssh://`. `pushToGit`'s schema already documented HTTPS; this makes that contract real. A non-https URL is refused with a message naming the scheme rather than the host, so an operator is not sent to a chart value that is already correct.

   Separately, and not an allowlist matter: `pushRepo` refused paths that escape the clone but not paths that stay **inside** it and reach `<clone>/.git/**`. `.git/config` is not data — pushRepo's own next `git add` reads it, so a `core.fsmonitor` entry there executes as the server (confirmed: a real clone, a real marker file, and a push that still reported success). Both callers reached it, `pushToGit` through `targetPath` + an AI-generated `relativePath`, and remediate's `git_create_pr` through an unvalidated model-supplied `files` array. The guard is in `pushRepo` so one place covers both, rejects any `.git` component of the **resolved** path (so a committed symlink to `.git` cannot launder it), folds case and strips trailing dots/spaces, and validates the whole batch before writing any of it.

   The original deferral reasoning follows. This is pre-existing behavior in v1.25.0, is not introduced or worsened by PR mode, and is not mitigated by it — which is why it was initially scoped out. It does mean the follow-up issue is a credential-exposure issue with an allowlist as one remedy, not a tidiness issue. Worth noting the codebase already solves the adjacent problem for PRD #621's per-request override tokens with a host-bound `GIT_ASKPASS` helper (`git-utils.ts:262-293`), keeping the token off the argv and out of `.git/config`; even that binds to the client-supplied host, so it defeats redirect-based leakage rather than a directly named attacker host. The env/GitHub-App path that `pushToGit` uses has neither protection.
