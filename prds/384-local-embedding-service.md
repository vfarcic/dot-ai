...
- [ ] Run existing integration tests as-is (still using OpenAI embeddings) to confirm the local embedding service deployment doesn't break anything
...
### Phase 3: Switch Tests to Local Embeddings

- [ ] Rebuild pre-populated Qdrant test image (`Dockerfile-qdrant`) with 384-dimension vectors generated by the local model (replaces current 1536-dimension OpenAI vectors in `capabilities-policies` collection)
- [ ] Update test infrastructure (`run-integration-tests.sh`) to deploy the local embedding service and remove OpenAI embedding key dependency
- [ ] All integration tests passing with local embeddings as the default (no external embedding API keys required)
