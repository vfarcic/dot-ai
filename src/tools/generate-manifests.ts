...
/**
 * PRD #392 Milestone 2 + PRD #710 Milestone 3: whether the caller holds 'apply'
 * on 'recommend' decides both whether deployManifests is offered and which of
 * the two pushToGit modes is presented, so it is resolved once per response and
 * handed to both option builders.
 */
async function isApplyAllowed(): Promise<boolean> {
  const rbacResult = await checkToolAccess(getCurrentIdentity(), {
    toolName: 'recommend',
    verb: 'apply',
  });
 ...
 * RBAC determines whether deploy is available and, per PRD #710 Milestone 3,
 * which push mode is offered.
 */
export function buildAgentInstructions(
  ...
  if (applyAllowed) {
    parts.push(
      ...
      '   Add pullRequest: true to open a pull request instead of committing to the branch directly — required if the target branch is protected. The head branch is generated by the server; "branch" then names the base branch the pull request targets.'
    );
  } else {
    // PRD #710 Milestone 3: direct push is a mutation gated on the same verb as
    // deployManifests, so an execute-only caller is offered PR mode only —
    // steered to the mode it can use instead of discovering the restriction
    // through a FORBIDDEN response.
    parts.push(
...
/** One entry of the machine-readable `nextActions` list. */
...
const SAVE_LOCALLY_ACTION: NextAction = {
  action: 'saveLocally',
  description:
    'Save files locally (no server call needed — files are in the response)',
};

const DEPLOY_MANIFESTS_ACTION: NextAction = {
  action: 'deployManifests',
  description: 'Apply directly to cluster',
  stage: 'deployManifests',
};

const PUSH_TO_GIT_ACTION: NextAction = {
  action: 'pushToGit',
  description:
    'Push to Git repository for GitOps (Argo CD, Flux). Pass pullRequest: true to commit to a server-generated branch and open a pull request against `branch` instead of pushing to it directly — required for a branch-protected repository (PRD #710)',
  stage: 'pushToGit',
  requiredParams: ['repoUrl', 'targetPath'],
  optionalParams: ['branch', 'commitMessage', 'pullRequest'],
};

/**
 * PRD #710 Milestone 3: the pushToGit variant offered to a caller without
 * 'apply' on 'recommend'. Direct push is a mutation and unreachable for them,
 * so pullRequest: true is presented as required rather than optional.
 */
const PUSH_TO_GIT_PR_ONLY_ACTION: NextAction = {
  action: 'pushToGit',
  description:
    "Open a pull request with these manifests for GitOps (Argo CD, Flux). pullRequest: true is required — pushing directly to `branch` needs 'apply' permission on 'recommend'. The head branch is generated by the server; `branch` is only the base branch the pull request targets (PRD #710)",
  stage: 'pushToGit',
  requiredParams: ['repoUrl', 'targetPath', 'pullRequest'],
  optionalParams: ['branch', 'commitMessage'],
};

/**
 * Build the nextActions list, offering only what the caller is permitted to do
 * (PRD #710 Milestone 3).
 *
 * deployManifests and a direct push are both gated on 'apply' on 'recommend',
 * so for an execute-only caller neither belongs here: an action a client
 * follows must never be a guaranteed FORBIDDEN. Deploy is dropped from the list
 * entirely rather than listed as unavailable, because every entry of
 * nextActions is something to invoke — buildAgentInstructions is where the
 * caller is TOLD deploy needs 'apply', and the two must keep agreeing.
 */
export function buildNextActions(applyAllowed: boolean): NextAction[] {
  return applyAllowed
    ? [SAVE_LOCALLY_ACTION, DEPLOY_MANIFESTS_ACTION, PUSH_TO_GIT_ACTION]
    : [SAVE_LOCALLY_ACTION, PUSH_TO_GIT_PR_ONLY_ACTION];
}
    ...
    try {
      ...
      if (validation.valid) {
        ...
        const response = {
          ...
          nextActions: [SAVE_LOCALLY_ACTION, DEPLOY_MANIFESTS_ACTION], // Helm: pushToGit not yet supported
        ...
        try {
          ...
          if (validation.valid) {
            ...
            if (outputFormat === 'helm' || outputFormat === 'kustomize') {
              ...
              // PRD #710 M3: one 'apply' check drives both option presentations
              const applyAllowed = await isApplyAllowed();
              ...
              const response = {
                ...
                packagingAttempts: packagingResult.attempts,
                timestamp: new Date().toISOString(),
                nextActions: buildNextActions(applyAllowed),
