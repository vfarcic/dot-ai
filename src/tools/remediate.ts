...
export const REMEDIATE_TOOL_INPUT_SCHEMA = {
  ...
  executedCommands: z
    .array(z.string())
    .optional()
    .describe('Commands that were executed to remediate the issue'),
  interaction_id: z
    .string()
    .optional()
    .describe(
...
async function conductInvestigation(
  ...
  try {
    ...
    const output: RemediateOutput = {
      status:
        finalAnalysis.issueStatus === 'active'
          ? 'awaiting_user_approval'
          : 'success',
      sessionId: session.sessionId,
      investigation: {
        iterations: result.iterations,
        dataGathered: result.toolCallsExecuted.map(
          (tc, i) => `${tc.tool} (call ${i + 1})`
        ),
      },
      analysis: {
    ...
    if (
      finalAnalysis.issueStatus === 'resolved' ||
      finalAnalysis.issueStatus === 'non_existent'
    ) {
      const statusMessage =
        finalAnalysis.issueStatus === 'resolved'
      ...
      const mediumRiskActions = finalAnalysis.remediation.actions.filter(
        a => a.risk === 'medium'
      );
      const riskSummary = [
        ...(highRiskActions.length > 0
          ? [
              `${highRiskActions.length} HIGH RISK actions require careful review`,
            ]
          : []),
        ...(mediumRiskActions.length > 0
          ? [
              `${mediumRiskActions.length} MEDIUM RISK actions should be executed with monitoring`,
            ]
          : []),
...
export function parseAIFinalAnalysis(
  ...
  try {
    ...
    throw new Error(
      `Failed to parse AI final analysis response: ${error instanceof Error ? error.message : 'Unknown error'}. Response content: "${aiResponse}"`,
      { cause: error }
    );
  }
}
...
async function executeUserChoice(
  ...
  try {
    ...
    logger.error('Choice execution failed', error as Error, {
      requestId,
      sessionId,
      choice,
    });
...
/**
 * A gitSource action that succeeded WITHOUT producing a pull request (PRD #710
 * decisions 3 and 7).
 */
export interface GitOpsWithoutPr {
...
export interface RemediationResponseShapeInput {
  ...
  /** kubectl/helm commands actually run. A gitSource action does not count. */
  executedCommandCount: number;
  /** Set only when a gitSource action opened a pull request. */
  pullRequestInfo?: RemediateOutput['pullRequest'];
  /** gitSource actions that succeeded without opening one. */
  gitOpsWithoutPr: GitOpsWithoutPr[];
  /** The final analysis's actions, in order — `command` may be absent. */
  actions: RemediationAction[];
   ...
   * earlier — so this selects wording, not a success claim.
   */
  validationAttempted: boolean;
}

/** The caller-facing half of the execution response that depends on the above. */
export interface RemediationResponseShape {
  message: string;
  nextSteps: string[];
  showExecutedCommands: boolean;
  showActualKubectlCommands: boolean;
}

/**
 * Decide how an executed remediation is reported: the summary message, the
 * next-steps list, and whether the agent should show executed kubectl commands.
 *
 * Three mutually exclusive stories, because a run is one of three shapes and the
 * kubectl story is a lie for two of them (PRD #710):
 * - GitOps with a PR (`executedCommandCount === 0`, `pullRequestInfo` set) —
 *   review and merge the PR, then wait for Argo CD/Flux.
 * - GitOps WITHOUT a PR — the manifests already matched the base branch
 *   (decision 3) or no pull request could be opened for the remote (decision 7).
 *   These leave `pullRequestInfo` undefined, so without this branch they fall
 *   through to the kubectl story and report an empty set of executed commands.
...
export function buildRemediationResponseShape(
  ...
  const hasOnlyGitOps =
    executedCommandCount === 0 && pullRequestInfo !== undefined;
  ...
  if (hasOnlyGitOps && pullRequestInfo) {
    nextSteps = [
      ...
      `  PR: ${pullRequestInfo.url}`,
      `  Branch: ${pullRequestInfo.branch} → ${pullRequestInfo.baseBranch}`,
      `  Files changed: ${pullRequestInfo.filesChanged.join(', ')}`,
      '',
      'Next steps:',
  ...
  return {
    message,
    nextSteps,
    // Both flags are about kubectl output, so both are off for the two GitOps
    // stories: there are no executed commands to show.
...
async function executeRemediationCommands(
   ...
   * the response falls through to the kubectl story and reports executed
   * commands it never ran — see buildRemediationResponseShape().
   */
  ...
  if (
    ...
    try {
      ...
      if (session.data.mode === 'automatic') {
        logger.info(
          'Waiting for Kubernetes to apply changes before validation',
          {
            requestId,
            sessionId: session.sessionId,
            delayMs: 30000,
...
IMPORTANT: You MUST respond with the final JSON analysis format as specified in your instructions. Verify the remediation was successful and return your analysis as JSON with issueStatus set to "resolved" if fixed, or "active" if issues remain.`;
      ...
      logger.info(
        'Validation confirmed issue is resolved, creating success response',
        {
          requestId,
          sessionId: session.sessionId,
          validationStatus: validationOutput.status,
  ...
  const response = {
    ...
    instructions: {
      showExecutedCommands: shape.showExecutedCommands,
      showActualKubectlCommands: shape.showActualKubectlCommands,
      nextSteps: shape.nextSteps,
    },
 ...
 */
export async function handleRemediateTool(
  args: RemediateInput
): Promise<RemediateToolResponse> {
  ...
  try {
    ...
    const sessionManager = new GenericSessionManager<RemediateSessionData>(
      'rem'
    );
    ...
    const isValidation =
      validatedInput.executedCommands &&
      validatedInput.executedCommands.length > 0;
    ...
    if (finalAnalysis.status === 'success') {
      logger.info(
        'Issue resolved/non-existent - returning success without execution decision',
        {
          requestId,
          sessionId: session.sessionId,
    ...
    const validated = {
      ...
      confidenceThreshold:
        args.confidenceThreshold !== undefined
          ? REMEDIATE_TOOL_INPUT_SCHEMA.confidenceThreshold.parse(
              args.confidenceThreshold
            )
          : 0.8,
      maxRiskLevel: args.maxRiskLevel
        ? REMEDIATE_TOOL_INPUT_SCHEMA.maxRiskLevel.parse(args.maxRiskLevel)
        : 'low',
      executeChoice:
        args.executeChoice !== undefined
          ? REMEDIATE_TOOL_INPUT_SCHEMA.executeChoice.parse(args.executeChoice)
          : undefined,
      sessionId: args.sessionId
        ? REMEDIATE_TOOL_INPUT_SCHEMA.sessionId.parse(args.sessionId)
        : undefined,
      executedCommands: args.executedCommands
        ? REMEDIATE_TOOL_INPUT_SCHEMA.executedCommands.parse(
            args.executedCommands
          )
        : undefined,
      interaction_id: args.interaction_id
        ? REMEDIATE_TOOL_INPUT_SCHEMA.interaction_id.parse(args.interaction_id)
        : undefined,
    } as RemediateInput;
