...
export class PatternComparativeEvaluator extends BaseComparativeEvaluator {
  ...
  readonly description =
    'Compares AI models on Kubernetes organizational pattern management quality';
  ...
  async evaluateAllScenarios(): Promise<ComparativeEvaluationScore[]> {
    try {
      ...
      console.log(
        `Found ${scenarios.length} pattern scenarios with multiple models for comparative evaluation`
      );
  ...
  getEvaluationPhases(): {
    ...
    const phaseDescriptions: Record<string, string> = {
      pattern_create_workflow:
        'Pattern Creation Workflow - How well each model guides users through creating organizational patterns',
      trigger_expansion:
        'Trigger Expansion Phase - How well each model expands infrastructure triggers for patterns',
      pattern_validation:
        'Pattern Validation Phase - How well each model validates organizational patterns',
      pattern_matching:
        'Pattern Matching Phase - How well each model matches user requirements to existing patterns',
    };
