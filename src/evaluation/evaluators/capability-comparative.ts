...
export class CapabilityComparativeEvaluator extends BaseComparativeEvaluator {
  ...
  readonly description =
    'Compares AI models on Kubernetes capability inference quality';
  ...
  async evaluateAllScenarios(): Promise<ComparativeEvaluationScore[]> {
    try {
      ...
      console.log(
        `Found ${scenarios.length} capability scenarios with multiple models for comparative evaluation`
      );
  ...
  getEvaluationPhases(): {
    ...
    const phaseDescriptions: Record<string, string> = {
      auto_scan:
        'Auto Scan Phase - How well each model analyzes cluster resource capabilities automatically',
      crud_auto_scan:
        'CRUD Auto Scan Phase - How well each model handles capability analysis with CRUD operations',
      list_auto_scan:
        'List Auto Scan Phase - How well each model handles capability listing and organization',
      search_auto_scan:
        'Search Auto Scan Phase - How well each model handles capability search and filtering',
    };
