...
import {
  ...
  ComparativeEvaluationScore,
} from './base-comparative.js';
...
export class PolicyComparativeEvaluator extends BaseComparativeEvaluator {
  ...
  readonly description =
    'Compares AI models on Kubernetes organizational policy intent management quality';
  ...
  async evaluateAllScenarios(): Promise<ComparativeEvaluationScore[]> {
    try {
      ...
      console.log(
        `Found ${scenarios.length} policy scenarios with multiple models for comparative evaluation`
      );
  ...
  getEvaluationPhases(): {
    ...
    const phaseDescriptions: Record<string, string> = {
      policy_create_workflow:
        'Policy Creation Workflow - How well each model guides users through creating organizational policies',
      policy_validation:
        'Policy Validation Phase - How well each model validates policy intent correctness and enforceability',
      policy_enforcement_recommendations:
        'Policy Enforcement Recommendations - How well each model provides enforcement strategies',
      policy_compliance_analysis:
        'Policy Compliance Analysis - How well each model analyzes compliance with existing policies',
    };
