...
export function loadEvaluationMetadata(): EvaluationMetadata {
  try {
    ...
    console.log(
      `✅ Loaded metadata for ${Object.keys(metadata.models || {}).length} models and ${Object.keys(metadata.tools || {}).length} tools`
    );
    return {
 ...
 */
export function buildModelPricingContext(
  models: Record<string, ModelMetadata>
): string {
  ...
  const pricingLines = modelIds.map(modelId => {
    ...
    const inputCost =
      model.pricing?.input_cost_per_million_tokens?.toFixed(2) || 'N/A';
    const outputCost =
      model.pricing?.output_cost_per_million_tokens?.toFixed(2) || 'N/A';
...
**Model Requirements**:
${Object.entries(tool.modelRequirements)
