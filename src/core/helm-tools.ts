...
export const HELM_LIST_TOOL: AITool = {
  name: 'helm_list',
  description:
    'List all Helm releases in the cluster. Returns release name, namespace, chart, version, status, and last updated time. Use this to discover what applications are managed by Helm and their current state. Essential for identifying Helm-managed applications before performing operations.',
  inputSchema: {
    type: 'object',
    properties: {
      namespace: {
        type: 'string',
        description:
          'Kubernetes namespace to list releases from. Omit to list releases across all namespaces.',
      },
      filter: {
        type: 'string',
        description:
          'Filter releases by name using a regex pattern (e.g., "prometheus" to find prometheus-related releases).',
      },
    },
    required: [],
  },
...
export const HELM_STATUS_TOOL: AITool = {
  name: 'helm_status',
  description:
    'Get detailed status of a specific Helm release including deployment status, revision number, last deployed time, and any notes from the chart. Use this to understand the current state of a Helm-managed application, especially when diagnosing issues or preparing for upgrades.',
  inputSchema: {
    type: 'object',
    properties: {
      release: {
        type: 'string',
        description:
          'Name of the Helm release to get status for (e.g., "prometheus", "argocd", "cert-manager").',
      },
      namespace: {
        type: 'string',
        description:
          'Kubernetes namespace where the release is deployed. Required for namespaced releases.',
      },
    },
    required: ['release'],
  },
...
export const HELM_HISTORY_TOOL: AITool = {
  name: 'helm_history',
  description:
    'Get the revision history of a Helm release showing all past deployments, upgrades, and rollbacks. Each revision includes status, chart version, app version, and description. Essential for identifying rollback targets and understanding deployment timeline when troubleshooting issues.',
  inputSchema: {
    type: 'object',
    properties: {
      release: {
        type: 'string',
        description:
          'Name of the Helm release to get history for (e.g., "prometheus", "argocd").',
      },
      namespace: {
        type: 'string',
        description:
      ...
      max: {
        type: 'number',
        description:
          'Maximum number of revisions to return. Defaults to 256 (Helm default).',
      },
    },
    required: ['release'],
  },
...
export const HELM_GET_VALUES_TOOL: AITool = {
  ...
  description:
    'Get the current custom values (user-supplied configuration) for a Helm release. Shows only values that differ from chart defaults. Use this to understand how a release was configured, preserve settings during upgrades, or diagnose configuration-related issues.',
  inputSchema: {
    type: 'object',
    properties: {
      release: {
        type: 'string',
        description:
          'Name of the Helm release to get values for (e.g., "prometheus", "argocd").',
      },
      namespace: {
        type: 'string',
        description:
      ...
      allValues: {
        type: 'boolean',
        description:
          'If true, returns all computed values (chart defaults + user values). Default is false (user values only).',
      },
    },
    required: ['release'],
  },
 ...
 * Follows the same pattern as kubernetes-utils.ts
 */
 ...
 * Uses argument escaping to prevent command injection (same pattern as kubernetes-utils.ts)
 */
async function executeHelm(args: string[]): Promise<string> {
  // Safely escape all arguments before joining
  const escapedArgs = args.map(arg => escapeShellArg(arg));
  const command = `helm ${escapedArgs.join(' ')}`;
  ...
  try {
    ...
    return (
      stdout + (stderr && !stderr.includes('WARNING') ? `\n${stderr}` : '')
    );
  } catch (error) {
 ...
 */
export async function executeHelmTools(
  toolName: string,
  input: any
): Promise<any> {
  try {
