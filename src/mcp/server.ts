...
async function main() {
  try {
        ...
        pluginManager.setOnPluginDiscovered(plugin => {
          ...
          if (plugin.tools.some(tool => tool.name === 'vector_store')) {
            runKnowledgeMigration(
              new ConsoleLogger('KnowledgeMigration')
            ).catch(err => {
              process.stderr.write(
                `Knowledge migration (post-discovery) error: ${err instanceof Error ? err.message : String(err)}\n`
              );
            });
          }
        });
    ...
    if (pluginConfigs.length > 0) {
      ...
      process.stderr.write(
        'Checking for legacy knowledge collections to migrate...\n'
      );
    ...
    if (mcpServerConfigs.length > 0) {
      process.stderr.write(
        `Discovering ${mcpServerConfigs.length} MCP server(s)...\n`
      );
      try {
        await mcpClientManager.discoverMcpServers(mcpServerConfigs);
        const stats = mcpClientManager.getStats();
        process.stderr.write(
          `MCP server discovery complete: ${stats.serverCount} server(s), ${stats.toolCount} tool(s)\n`
        );
      } catch (error) {
        ...
        process.stderr.write(
          `FATAL: MCP server discovery failed: ${error instanceof Error ? error.message : String(error)}\n`
        );
        process.exit(1);
      }
      ...
      process.stderr.write(
        'No MCP servers configured (mount mcp-servers.json at /etc/dot-ai-mcp/mcp-servers.json to enable)\n'
      );
    }
