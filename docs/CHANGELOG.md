...
- **Client-Driven OAuth Token Expiry**
  ...
  Configure default and maximum token expiry using environment variables: `OAUTH_DEFAULT_TOKEN_TTL_SECONDS` (default: 86400 / 1 day) and `OAUTH_MAX_TOKEN_TTL_SECONDS` (default: 7776000 / 90 days). The server transparently honors client requests within these limits, with invalid or excessive values falling back to the default expiry. ([#380](https://github.com/vfarcic/dot-ai/issues/380))
...
- **GitOps Remediation via Pull Requests**
  ...
  Requires a GitHub token with PR creation permissions configured via `DOT_AI_GIT_TOKEN` or GitHub App credentials. ([#408](https://github.com/vfarcic/dot-ai/issues/408))
...
- **MCP OAuth Authentication & User Identity**
  ...
  See the [Authentication Overview](https://devopstoolkit.ai/docs/mcp/setup/authentication), [Connector Configuration](https://devopstoolkit.ai/docs/mcp/setup/connectors), and [MCP Client Setup](https://devopstoolkit.ai/docs/mcp/) for details. ([#380](https://github.com/vfarcic/dot-ai/issues/380))
...
- **On-Demand Prompts Cache Refresh**
  ...
  If no user prompts repository is configured, the endpoint succeeds gracefully and reports `source: "built-in"` in the response. ([#386](https://github.com/vfarcic/dot-ai/issues/386))
