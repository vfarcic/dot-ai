...
Supported token formats: `gho_*` (OAuth, recommended) and `ghu_*` (GitHub App). Personal access tokens (`github_pat_*` fine-grained PATs and `ghp_*` classic PATs) are not supported by `api.githubcopilot.com`. The resolver checks `GITHUB_COPILOT_TOKEN`, `GH_TOKEN`, and `GITHUB_TOKEN` env vars in that priority order, taking the first one holding a *supported* token — an unsupported token in an earlier variable does not mask a usable one in a later variable. Note: the `gh auth token` CLI fallback is not available in Kubernetes deployments where `gh` is not installed — supply the token explicitly via the env var.

**`GH_TOKEN` and `GITHUB_TOKEN` are general-purpose GitHub credentials**, typically set for the `gh` CLI, GitHub Actions, or Terraform rather than for Copilot. If either holds a `gho_*`/`ghu_*` token, the resolver will use it as the Bearer credential against `api.githubcopilot.com` — an audience it may not have been issued for. This is deliberate, because it makes `gh auth login` work as a zero-config local setup, but it means a token provisioned for one purpose can be spent on Copilot inference. To keep the credential explicit, set `GITHUB_COPILOT_TOKEN` and leave the other two unset. In Kubernetes this is the default: the chart maps only `secrets.copilot.token` into `GITHUB_COPILOT_TOKEN` ([`deployment.yaml`](https://github.com/vfarcic/dot-ai/blob/main/charts/templates/deployment.yaml)), so `GH_TOKEN` and `GITHUB_TOKEN` reach a pod only if you add them yourself via `extraEnv`.
