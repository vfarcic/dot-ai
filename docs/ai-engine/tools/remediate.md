...
**A private repository needs an `https://` URL on an allowlisted host.** The repository URL comes from the source the tool discovered in the cluster, so the server only sends **its own** Git credential to an `https://` URL whose host is listed in [`gitops.allowedRepoHosts`](../setup/deployment.md#what-the-allowlist-gates) (default `["github.com", "www.github.com"]`). A public repository is cloned as before, whatever its host; a **private** one reached by an unlisted host — or by an `ssh://` or scp-style URL, which cannot carry the credential at all — is cloned unauthenticated and therefore fails, reporting git's own credential error rather than an allowlist message. Ask your operator to add the host; there is no per-request credential for this path.
