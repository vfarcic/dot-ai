...
describe('SolutionEnhancer', () => {
  let enhancer: SolutionEnhancer;
  let mockConfig: AIRankingConfig;
  let mockSolution: ResourceSolution;
  let mockExplainResource: jest.MockedFunction<any>;
  ...
  beforeEach(() => {
    ...
    mockSolution = {
      ...
      score: 80,
      description: 'Simple Pod deployment',
    ...
    mockExplainResource = jest.fn().mockResolvedValue({
      kind: 'Pod',
      version: 'v1',
      group: '',
    ...
    enhancer = new SolutionEnhancer(mockConfig);
  });
  ...
  describe('Constructor and Initialization', () => {
    test('should initialize with valid config', () => {
      ...
      expect(enhancer).toBeInstanceOf(SolutionEnhancer);
    });

    test('should handle missing API key gracefully', () => {
      const invalidConfig = { claudeApiKey: '' };
      expect(() => new SolutionEnhancer(invalidConfig)).toThrow('API key is required for Claude integration');
    });
  });

  describe('enhanceSolution', () => {
    test('should throw error when Claude integration not initialized', async () => {
      // Mock ClaudeIntegration to return false for isInitialized
      const mockClaudeIntegration = {
        isInitialized: jest.fn().mockReturnValue(false),
        sendMessage: jest.fn()
      };
      
      (enhancer as any)['claudeIntegration'] = mockClaudeIntegration;
      
      await expect(
        enhancer.enhanceSolution(
          mockSolution,
    ...
    test('should handle basic enhancement request', async () => {
        ...
        sendMessage: jest.fn()
          .mockResolvedValueOnce({
            content: JSON.stringify({
              ...
              reasoning: 'Pod can handle scaling requirements',
              requiredResources: ['Pod'],
              requiredCapabilities: ['resources.limits']
            })
          })
          .mockResolvedValueOnce({
            content: JSON.stringify({
      ...
      // Replace the ClaudeIntegration instance
      (enhancer as any)['claudeIntegration'] = mockClaudeIntegration;

      const enhancedSolution = await enhancer.enhanceSolution(
        mockSolution,
        'I need more CPU for high performance',
        { resources: [{ kind: 'Pod', group: '', version: 'v1' }], custom: [] },
        mockExplainResource
      );
    ...
    test('should handle capability gap scenario', async () => {
        ...
        sendMessage: jest.fn()
          .mockResolvedValueOnce({
            content: JSON.stringify({
              ...
              reasoning: 'Current resources cannot handle load balancing',
              suggestedAction: 'Consider using Service and Ingress resources'
            })
          })
      };
      ...
      ).rejects.toThrow('Enhancement capability gap');
    });

    test('should preserve original solution properties', async () => {
      const mockClaudeIntegration = {
        isInitialized: jest.fn().mockReturnValue(true),
        sendMessage: jest.fn()
          .mockResolvedValueOnce({
      ...
      expect(enhancedSolution.reasons).toEqual(mockSolution.reasons);
      expect(enhancedSolution.analysis).toBe(mockSolution.analysis);
      // Resources may have Map converted to Object during JSON serialization
      expect(enhancedSolution.resources).toHaveLength(mockSolution.resources.length);
      expect(enhancedSolution.resources[0].kind).toBe(mockSolution.resources[0].kind);
    });
    ...
    test('should handle AI service failures gracefully', async () => {
      ...
      ).rejects.toThrow('Solution enhancement failed');
    });
    ...
    test('should handle multiple resource types', async () => {
      const multiResourceSolution: ResourceSolution = {
        ...mockSolution,
        type: 'combination',
        resources: [
          mockSolution.resources[0],
          {
            apiVersion: 'v1',
            kind: 'Service',
            group: '',
      ...
      const mockClaudeIntegration = {
        ...
        sendMessage: jest.fn()
          .mockResolvedValueOnce({
            content: JSON.stringify({
              approach: 'enhance_existing',
              reasoning: 'Multiple resources can be enhanced',
              requiredResources: ['Pod', 'Service'],
      ...
      // Mock explainResource to handle multiple calls
      mockExplainResource
        .mockResolvedValueOnce({
          kind: 'Pod',
          version: 'v1',
      ...
      expect(enhancedSolution).toBeDefined();
      expect(enhancedSolution.resources).toHaveLength(2);
      expect(mockExplainResource).toHaveBeenCalledTimes(2);
    });

    test('should handle network errors during schema fetching', async () => {
      mockExplainResource.mockRejectedValue(new Error('Network timeout'));
