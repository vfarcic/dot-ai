...
describe('capabilities list/progress/delete response contract (PRD #714)', () => {
  ...
  it('list — rejects a non-integer limit instead of silently defaulting', async () => {
    const result = await handleCapabilityList(
      { limit: 10.5 },
      logger,
      FIXED_REQUEST_ID,
  ...
  it('list — rejects a non-boolean identityOnly value', async () => {
    const result = await handleCapabilityList(
      { identityOnly: 'true' as unknown as boolean },
      logger,
      FIXED_REQUEST_ID,
      fakeService({})
    );
  ...
  it('list — does not publish an empty snapshot when collection status fails', async () => {
    vi.mocked(CapabilityVectorService).mockImplementation(function () {
      return fakeService({
        healthCheck: async () => true,
        collectionExists: async () => {
          throw new Error('collection status unavailable');
        },
      });
    } as unknown as () => CapabilityVectorService);
    ...
    expect(result).toMatchObject({
      ...
      dataType: 'capabilities',
      error: {
        message: 'Capability list failed',
        details: 'collection status unavailable',
      },
    });
  });
