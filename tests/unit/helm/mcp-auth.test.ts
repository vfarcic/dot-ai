...
describe.concurrent(
  'MCP Server Authentication Helm Templates (PRD #414)',
  () => {
    test('no auth config produces no MCP_AUTH env vars (backward compatible)', () => {
      ...
      const mcpAuthEnvs = container!.env?.filter(
        e => e.name.startsWith('MCP_AUTH_') || e.name.startsWith('MCP_HEADERS_')
      );
    ...
    test('auth.token.existingSecret injects MCP_AUTH_* env var from Secret', () => {
      ...
      const authEnv = container!.env?.find(
        e => e.name === 'MCP_AUTH_CONTEXT_FORGE'
      );
    ...
    test('auth.headers.existingSecret injects MCP_HEADERS_* env var from Secret', () => {
      ...
      const headersEnv = container!.env?.find(
        e => e.name === 'MCP_HEADERS_LEGACY_SERVER'
      );
    ...
    test('MCP servers ConfigMap includes auth env var names in JSON', () => {
      ...
      const configMaps = findResourcesByKind<ConfigMapResource>(
        docs,
        'ConfigMap'
      );
      const mcpCm = configMaps.find(cm =>
        cm.metadata.name.includes('mcp-servers')
      );
    ...
    test('both token and headers auth on same server', () => {
      ...
      const tokenEnv = container!.env?.find(
        e => e.name === 'MCP_AUTH_MULTI_AUTH'
      );
      const headersEnv = container!.env?.find(
        e => e.name === 'MCP_HEADERS_MULTI_AUTH'
      );
    ...
    test('auth.oauth.existingSecret injects MCP_OAUTH_SECRET_* env var from Secret (M4)', () => {
      ...
      const oauthEnv = container!.env?.find(
        e => e.name === 'MCP_OAUTH_SECRET_UPSTREAM_AI'
      );
    ...
    test('OAuth config in ConfigMap includes clientId, clientSecretEnvVar, and scope (M4)', () => {
      ...
      expect(config[0].auth.oauth.clientSecretEnvVar).toBe(
        'MCP_OAUTH_SECRET_UPSTREAM_AI'
      );
