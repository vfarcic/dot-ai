...
describe.concurrent('PRD #375 Unified Knowledge Base - Migration & E2E', () => {
  ...
  describe.concurrent('E2E: Ingest, Classification, and Search', () => {
    test('should classify policy document and return tags in search results', async () => {
      ...
      const ingestResponse = await callTool(
    ...
    test('should classify pattern document and return tags in search results', async () => {
      const testId = Date.now();
    ...
    test('should assign empty tags to general (non-policy, non-pattern) document', async () => {
      const testId = Date.now();
    ...
    test('should handle empty document gracefully during ingestion', async () => {
      const testId = Date.now();
  ...
  describe.concurrent('E2E: Consumer Tools See Unified Knowledge', () => {
    test('should allow operate tool to find context from unified knowledge base', async () => {
      const testId = Date.now();
      ...
      expect(searchResponse).toMatchObject({
        ...
        data: {
          result: {
            success: true,
            chunks: expect.arrayContaining([expect.objectContaining({ uri })]),
          },
        },
      });
  ...
  describe.concurrent(
    'Migration: Legacy patterns → Unified knowledge base',
    () => {
