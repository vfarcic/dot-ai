...
describe.concurrent('Version Tool Integration', () => {
  ...
  describe('System Status via REST API', () => {
    test('should return comprehensive system status with correct structure', async () => {
        ...
        data: {
          ...
          result: {
            ...
            system: {
              ...
              // PRD #343 + #251: Plugin stats (36 tools: 17 kubectl + 8 helm + 10 vector + 1 knowledge)
              plugins: {
                pluginCount: 1,
                toolCount: 36,
                plugins: [
      ...
      // 36 tools in agentic-tools plugin (17 kubectl + 8 helm + 10 vector + 1 knowledge - PRD #343, #251, #359, #356)
      expect(plugins?.toolCount, `Expected 36 tools, found ${plugins?.toolCount}. Check agentic-tools registration`).toBe(36);
