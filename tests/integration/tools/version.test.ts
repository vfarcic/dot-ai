...
describe.concurrent('Version Tool Integration', () => {
  ...
  describe('System Status via REST API', () => {
    test('should return comprehensive system status with correct structure', async () => {
        ...
        data: {
          ...
          result: {
            ...
            system: {
              ...
              // PRD #343: Plugin stats (29 tools: 17 kubectl + 4 helm + 8 vector - PRD #359)
              plugins: {
                pluginCount: 1,
                toolCount: 29,
                plugins: [
      ...
      // 29 tools in agentic-tools plugin (17 kubectl + 4 helm + 8 vector - PRD #359)
      expect(plugins?.toolCount, `Expected 29 tools, found ${plugins?.toolCount}. Check agentic-tools registration`).toBe(29);
