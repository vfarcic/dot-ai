...
describe('Resource Sync Endpoint Integration', () => {
  ...
  describe('Complete Sync Workflow', () => {
    test('should complete full resource sync workflow: upsert, verify, delete', async () => {
      ...
      const upsertResponse = await integrationTest.httpClient.post(
      ...
      expect(
        ...
        `Upsert failed: ${JSON.stringify(upsertResponse.error)}`
      ).toBe(true);
      ...
      const updateResponse = await integrationTest.httpClient.post(
        '/api/v1/resources/sync',
        {
          upserts: [
            {
      ...
      const idempotentDeleteResponse = await integrationTest.httpClient.post(
        '/api/v1/resources/sync',
        {
          deletes: [
            {
      ...
      expect(idempotentDeleteResponse).toMatchObject(
        expectedIdempotentDeleteResponse
      );

      // Step 5: Clean up - Delete remaining resource
      const cleanupResponse = await integrationTest.httpClient.post(
        '/api/v1/resources/sync',
        {
          deletes: [
            {
  ...
  describe('Resync Workflow', () => {
    test('should perform diff-based resync correctly', async () => {
      ...
      const initialSyncResponse = await integrationTest.httpClient.post(
        '/api/v1/resources/sync',
        {
          upserts: [
            {
  ...
  describe('Validation and Error Handling', () => {
    test('should reject invalid request body', async () => {
      const invalidResponse = await integrationTest.httpClient.post(
        '/api/v1/resources/sync',
        {
          upserts: [
            {
    ...
    test('should handle empty sync request gracefully', async () => {
      const emptyResponse = await integrationTest.httpClient.post(
        '/api/v1/resources/sync',
        {
          upserts: [],
          deletes: [],
    ...
    test('should reject non-POST methods', async () => {
      const getResponse = await integrationTest.httpClient.get(
        '/api/v1/resources/sync'
      );
