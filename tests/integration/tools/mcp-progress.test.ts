...
describe.concurrent('MCP Progress Notifications (PRD #705)', () => {
  ...
  test('semantic phases: recommend reports every phase including the question-generation loop, and still returns its result', async () => {
  ...
  test('heartbeat only: a toolLoop tool with no semantic phases still emits progress for liveness', async () => {
    const notifications: Progress[] = [];
  ...
  test('opt-out: no progressToken means no notifications and an unchanged result', async () => {
