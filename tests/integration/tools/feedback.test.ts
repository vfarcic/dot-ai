  ...
  describe('Feedback Message Statistical Verification', () => {
    test('should show feedback message approximately 5% of the time with default settings', async () => {
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        // Check if response contains feedbackMessage field in JSON
        if (response.success && response.data?.result) {
          const result = typeof response.data.result === 'string'
            ? JSON.parse(response.data.result)
            : response.data.result;

          // feedbackMessage field is present when feedback should be shown
          if (result.feedbackMessage &&
              result.feedbackMessage.includes('Help us improve dot-ai!') &&
              result.feedbackMessage.includes('forms.gle')) {
            feedbackCount++;
          }
        }
      }
